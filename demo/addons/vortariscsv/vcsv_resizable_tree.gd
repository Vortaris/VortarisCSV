@tool
class_name VCSVResizableTree
extends Tree
## Tree with user-draggable column separators for the VortarisCSV editor.
##
...
## themselves stay selectable / editable everywhere except the ~6px separator band.
##
## Adapted from VortarisModLoader's `VMLResizableTree` (same implementation).
...
## v_separation gutters + sort-arrow gutter). Kept as a public helper so tests /
## callers can target the header row.
