...
- **v0.4.0**: the CSV main-screen table is now an **Excel-style self-drawn grid** (`VCSVGrid`) — real cell borders, row-number gutter, virtual scrolling, click-header-to-sort, drag-to-resize, cell/rectangle selection with Ctrl/Cmd+C + right-click copy, and double-click editing. Cell-edit write-back is atomic (temp + rename) and reimports via `update_file()` (no progress-dialog errors). Strict parsing gets an error budget (`VCSVParseOptions.max_errors`), and `VCSVWriter.encoding` adds `utf8_bom` / `gbk` output (symmetric with the GBK parser, so GBK files round-trip). Parse/import logs are gated behind `vortariscsv/general/verbose`.
